...
Let users connect Rapora to their AI agents / LLMs (Claude Desktop, custom
...
- Raw token format: `rapora_pat_<random>`. Shown **once** at creation, never retrievable again.
...
>
> - **`GetProfileAndOrg` middleware** detects a `rapora_pat_` token (Bearer or
...
- Reads `Authorization: Bearer rapora_pat_…` (also accepts the existing `Token:` header style for parity).
...
- **stdio** (`rapora-mcp` console script via `uvx`/`pipx`): reads `RAPORA_BASE_URL` +
  `RAPORA_TOKEN` from env. Drops into Claude Desktop's `mcpServers` config. **Ships first.**
- **Streamable HTTP** (`rapora-mcp --http`, hosted e.g. at `mcp.rapora.io`): PAT
...
2. **Phase 2:** Streamable HTTP transport hosted at `mcp.rapora.io`; per-token
