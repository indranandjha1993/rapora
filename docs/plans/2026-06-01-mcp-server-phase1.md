# Rapora MCP Server — Phase 1 Implementation Plan
...
**Goal:** Ship a Personal Access Token (PAT) auth layer in the Django backend and a standalone Python FastMCP server (stdio) that lets a user connect their AI agent to Rapora with full CRUD over the existing REST API, acting as the token's owning user.
...
class TestPersonalAccessToken:
    def test_generate_returns_raw_token_and_persists_hash(self, org_a, admin_profile):
        ...
        assert raw.startswith("rapora_pat_")
...
def generate_pat_raw():
    ...
    return f"rapora_pat_{secrets.token_urlsafe(32)}"
...
class PersonalAccessToken(BaseOrgModel):
    ...
    def generate(cls, profile, name, scopes=None, expires_at=None):
        ...
        pat = cls.objects.create(
            ...
            token_prefix=raw[:13],  # rapora_pat_ + 4 chars only — avoid persisting secret bytes
            scopes=scopes or [],
...
**Why first in the list:** JWT auth also reads `Authorization: Bearer …` and *raises* on a token it can't decode. `PATAuthentication` must run first and only *claim* tokens starting with `rapora_pat_`, returning `None` for anything else so JWT/org-key still work.
...
class TestPATAuthentication:
    ...
    def test_unknown_pat_raises(self):
        req = self.factory.get("/api/leads/", HTTP_AUTHORIZATION="Bearer rapora_pat_nope")
        with pytest.raises(AuthenticationFailed):
            self.auth.authenticate(req)
...
PAT_PREFIX = "rapora_pat_"


def _extract_raw(request):
    """Pull a rapora_pat_ token from Authorization: Bearer or Token header."""
...
class PATAuthenticationScheme(OpenApiAuthenticationExtension):
    ...
    def get_security_definition(self, auto_schema):
        return {
            "type": "http",
            "scheme": "bearer",
            "description": "Personal access token (rapora_pat_…) for agent/MCP access",
        }
```
...
class TestPATApi:
    ...
    def test_create_returns_raw_token_once(self, client, admin_profile, auth_headers):
        ...
        body = resp.json()
        assert body["token"].startswith("rapora_pat_")  # raw token present ONCE
...
- Create: `mcp_server/src/rapora_mcp/__init__.py`
- Create: `mcp_server/src/rapora_mcp/config.py`
...
name = "rapora-mcp"
version = "0.1.0"
description = "Rapora MCP server — connect your AI agent to Rapora"
requires-python = ">=3.11"
dependencies = ["fastmcp>=2.0", "httpx>=0.27", "pydantic>=2.7"]

[project.scripts]
rapora-mcp = "rapora_mcp.server:main"
...
class Settings:
    ...
    @classmethod
    def from_env(cls):
        base = os.environ.get("RAPORA_BASE_URL", "").rstrip("/")
        token = os.environ.get("RAPORA_TOKEN", "")
        if not base or not token:
            raise SystemExit("RAPORA_BASE_URL and RAPORA_TOKEN env vars are required")
        return cls(base_url=base, token=token)
```

**Step 2:** `cd mcp_server && uv sync` → resolves deps, creates `.venv`.

**Step 3: Commit** *(USER)* `chore(mcp): scaffold rapora-mcp package`.
...
**Files:**
- Create: `mcp_server/src/rapora_mcp/client.py`
- Test: `mcp_server/tests/test_client.py`

**Step 1: Write failing test** (uses `respx` to mock httpx):
```python
import httpx, pytest, respx
from rapora_mcp.client import CrmClient, CrmError


@pytest.mark.asyncio
async def test_get_sends_bearer_and_returns_json():
    client = CrmClient("https://crm.example.com", "rapora_pat_abc")
    with respx.mock:
    ...
    assert data == {"results": []}
    assert route.calls.last.request.headers["Authorization"] == "Bearer rapora_pat_abc"
...
**Files:**
- Create: `mcp_server/src/rapora_mcp/entities.py`
- Test: `mcp_server/tests/test_entities.py`

**Step 1: Test:**
```python
from rapora_mcp.entities import ENTITIES, resolve_path, EntityError
import pytest
...
**Files:**
- Create: `mcp_server/src/rapora_mcp/tools.py`
...
from rapora_mcp.config import Settings
...
```json
{
  "mcpServers": {
    "rapora": {
      "command": "uvx",
      "args": ["rapora-mcp"],
      "env": { "RAPORA_BASE_URL": "http://localhost:8000", "RAPORA_TOKEN": "rapora_pat_…" }
    }
  }
}
```
...
- Streamable HTTP transport / hosted `mcp.rapora.io`
- OAuth 2.1 flow
