...
from rapora_mcp.config import HTTP, STDIO, Settings

_KEYS = ("RAPORA_BASE_URL", "RAPORA_TOKEN", "RAPORA_TRANSPORT",
         "RAPORA_HOST", "RAPORA_PORT", "RAPORA_PATH")
...
def test_stdio_requires_token(monkeypatch):
    _clear(monkeypatch)
    monkeypatch.setenv("RAPORA_BASE_URL", "http://localhost:8000")
    with pytest.raises(SystemExit):
        Settings.from_env()
...
def test_stdio_with_token_ok_and_strips_trailing_slash(monkeypatch):
    _clear(monkeypatch)
    monkeypatch.setenv("RAPORA_BASE_URL", "http://localhost:8000/")
    monkeypatch.setenv("RAPORA_TOKEN", "rapora_pat_x")
    s = Settings.from_env()
    assert s.transport == STDIO
    assert s.base_url == "http://localhost:8000"
    assert s.token == "rapora_pat_x"
...
def test_http_rejects_server_side_token(monkeypatch):
    ...
    _clear(monkeypatch)
    monkeypatch.setenv("RAPORA_BASE_URL", "http://localhost:8000")
    monkeypatch.setenv("RAPORA_TRANSPORT", "http")
    monkeypatch.setenv("RAPORA_TOKEN", "rapora_pat_x")
    with pytest.raises(SystemExit):
        Settings.from_env()
...
def test_http_without_token_ok_and_transport_case_insensitive(monkeypatch):
    ...
    monkeypatch.setenv("RAPORA_BASE_URL", "http://localhost:8000")
    monkeypatch.setenv("RAPORA_TRANSPORT", "HTTP")
    monkeypatch.setenv("RAPORA_PORT", "9001")
    s = Settings.from_env()
...
def test_invalid_transport_exits(monkeypatch):
    _clear(monkeypatch)
    monkeypatch.setenv("RAPORA_BASE_URL", "http://localhost:8000")
    monkeypatch.setenv("RAPORA_TRANSPORT", "websocket")
    with pytest.raises(SystemExit):
        Settings.from_env()
...
def test_non_integer_port_exits(monkeypatch):
    ...
    monkeypatch.setenv("RAPORA_BASE_URL", "http://localhost:8000")
    monkeypatch.setenv("RAPORA_TRANSPORT", "http")
    monkeypatch.setenv("RAPORA_PORT", "not-a-number")
    with pytest.raises(SystemExit):
        Settings.from_env()
