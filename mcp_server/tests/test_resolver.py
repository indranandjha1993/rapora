...
from rapora_mcp.auth import AuthError
from rapora_mcp.config import Settings
from rapora_mcp.server import ClientResolver
...
def _stdio_settings():
    return Settings(
        base_url="https://crm.example.com", token="rapora_pat_srv", transport="stdio"
    )
...
def test_stdio_builds_once_and_reuses():
    ...
    assert a._headers["Authorization"] == "Bearer rapora_pat_srv"
