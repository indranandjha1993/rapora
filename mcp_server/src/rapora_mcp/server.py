"""FastMCP server entry point for Rapora (stdio or http transport).

Wraps the plain async tool functions in ``rapora_mcp.tools`` as FastMCP tools and
exposes a ``build_server(client=None)`` factory plus a ``main()`` console entry
point (the ``rapora-mcp`` script points at ``rapora_mcp.server:main``).

The transport is chosen by ``RAPORA_TRANSPORT`` (default ``stdio``):

* **stdio** — the process acts as a single user; one :class:`CrmClient` is
  built lazily from ``RAPORA_BASE_URL`` + ``RAPORA_TOKEN`` and reused.
* **http** — a hosted, multi-user server; each request authenticates with its
  ...
  See :class:`ClientResolver` and ``rapora_mcp.auth``.
...
from rapora_mcp.auth import AuthError, extract_bearer_token
from rapora_mcp.client import CrmClient
from rapora_mcp.config import HTTP, Settings
        ...
        if self._settings.transport == HTTP:
            ...
            if not token:
                raise AuthError(
                    ...
                    "'Authorization: Bearer <rapora_pat_…>' with your request."
                )
...
def build_server(client=None, settings_loader=None):
    ...
    ``RAPORA_TRANSPORT`` being set in the Django process env.
    """
    mcp = FastMCP("Rapora")
...
def main():
    """Console entry point. Transport is selected by ``RAPORA_TRANSPORT``.

    ``stdio`` (default) talks over stdin/stdout for a locally-launched server.
    ``http`` serves streamable-HTTP on ``RAPORA_HOST``:``RAPORA_PORT`` at
    ``RAPORA_PATH`` and authenticates each request from its Authorization header.
    """
