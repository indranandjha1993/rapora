...
It also optionally mounts the Rapora **MCP server** at ``/mcp`` so AI agents
...
the optional ``rapora-mcp`` dependency is not installed, or ``RAPORA_MCP_ENABLED``
...
def _mcp_disabled():
    return os.environ.get("RAPORA_MCP_ENABLED", "true").strip().lower() in (
        "0",
        "false",
        "no",
        "off",
...
def _build_application():
    ...
    try:
        from rapora_mcp.auth import extract_bearer_token
        from rapora_mcp.server import build_http_app
    except ImportError:
    ...
    # address.
    base_url = os.environ.get("RAPORA_BASE_URL", "http://127.0.0.1:8000")
...
async def _unauthorized(scope, send):
    ...
    body = (
        ...
        b'header. Send: Authorization: Bearer <rapora_pat_...>"}'
    )
    await send(
        {
