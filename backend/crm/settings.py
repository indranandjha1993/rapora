...
# Generic SMTP settings (used when EMAIL_BACKEND is the SMTP backend, e.g.
# django.core.mail.backends.smtp.EmailBackend). For providers like Hostinger:
# host smtp.hostinger.com, port 465 with EMAIL_USE_SSL=True (or 587 with TLS).
...
# Platform super admins are identified by this email domain. Configure per
# deployment so super-admin access is not tied to any single hardcoded domain.
SUPERADMIN_EMAIL_DOMAIN = os.environ.get("SUPERADMIN_EMAIL_DOMAIN", "rapora.app")
