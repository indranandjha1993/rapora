...
class TestPermissionClasses(MultiTenancyBaseTestCase):
    ...
    def test_is_super_admin_with_super_email(self):
        """IsSuperAdmin allows the configured super-admin email domain."""
        ...
        user.email = f"admin@{settings.SUPERADMIN_EMAIL_DOMAIN}"
        request = MagicMock()
        request.user = user
        perm = IsSuperAdmin()
        self.assertTrue(perm.has_permission(request, None))
    ...
    def test_is_super_admin_regular_email(self):
        """IsSuperAdmin denies emails outside the configured domain."""
        from common.permissions import IsSuperAdmin
