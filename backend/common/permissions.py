...
class IsSuperAdmin(permissions.BasePermission):
    ...
    Super admins are identified by the email domain configured in
    settings.SUPERADMIN_EMAIL_DOMAIN. They have access to the admin panel
    ...
    def has_permission(self, request, view):
        ...
        # Check for super admin email domain (configurable per deployment)
        domain = getattr(settings, "SUPERADMIN_EMAIL_DOMAIN", "rapora.app")
        return bool(request.user.email) and request.user.email.endswith(f"@{domain}")
