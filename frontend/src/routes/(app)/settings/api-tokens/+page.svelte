<script>
  ...
  // Real, ready-to-paste API host (e.g. https://api.rapora.io). The MCP
  // client appends /api/... itself, so this is exactly RAPORA_BASE_URL.
  const baseUrl = $derived(data.baseUrl || 'https://api.rapora.io');
  ...
  const tokenValue = $derived(form?.created?.token || 'rapora_pat_…paste-your-token');
...
<svelte:head>
  <title>API Tokens - Settings - Rapora</title>
</svelte:head>

<PageHeader
  title="API Tokens"
  subtitle="Connect your AI agent to Rapora. Tokens act as you and inherit your role."
/>
      ...
      {#if helpOpen}
        <div class="space-y-3 border-t border-[var(--border-default)] p-4">
          ...
          <p class="text-sm text-[var(--text-secondary)]">
            ...
            {:else}
              Replace <code class="font-mono text-xs">RAPORA_TOKEN</code> with a token you created above.
            {/if}
          </p>
